...
export const atualizarClinica = async (
  ...
  const { endereco, email, senha, planoDeSaudeAceitos } = req.body;
...
export const listaEspecialistasPorClinica = async (
  req: Request,
  res: Response
): Promise<Response> => {
  const { id } = req.params;
...
export const atualizaEspecialistaPeloIdDaClinica = async (
  req: Request,
  res: Response
): Promise<Response> => {
  const { id } = req.params;
